...
| **Rebuild Phase 0** | Parent writes graph-backbone digests; **Opus-exclusive** inventory (`claude-opus-5-thinking-high`) — not Terra+Sonnet dual. Optional Opus structure pass; never second family for inventory cross-check |
...
| **Reviews** | Routine phase: Terra Loop A + Sonnet Loop B + Sonnet quality. Production/go-live: Sol + Fable + Fable quality. Trust-boundary Sol primary. Autonomous phase gates: **spawn Fable**. Checklist + evidence + review-admin + fix-canary |
...
| **Multi-model subagents** | Full protocol for rebuild/redesign + production-merge loops; routine phase = Everyday dual-family; inventory = Opus only; single-model elsewhere unless "use more models" |
| **Default models** | Canonical: `_meta/model-roster.json`. **UI default = Terra** (pending Terra vs Grok builder duel). **Grill me = Opus** (switch gate). **Autonomous build = Grok high**. **Autonomous review = spawn Fable**. Opus/Fable are event models |
| **Wrong parent model** | Judgment jobs → **spawn** Job slug. **Exception: full grill / grill me** → **STOP** and tell user to switch to Opus 5 (do not spawn grill) |
| **Autonomous models** | Grok (`cursor-grok-4.5-high`) implements; spawn Fable for phase review → fix. Go-live still Premier loops |
...
| **Grill (planning)** | Spec gate → mini-grill (Terra). **grill me** / full grill → Opus 5 parent required (`grill-protocol.mdc`) |
