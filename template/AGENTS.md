...
- **Trust-boundary security review** (anything touching payments/auth/roles/orders): review by entry point, not by file. For each public route / user action / staff action / webhook / cron, confirm actor + auth proof, server-side input validation, server-derived money/role/ownership (never trusted from client), side effects, and one abuse test. Real holes shipped past file-by-file reviews (public checkout marking an order paid, setup creating a dev with no secret, draft creation trusting client IDs).
- **Invariant ledger:** for money/auth/inventory/shipping/order structure, keep numbered invariants and have reviews cite invariant IDs, so they can't drift between audit, plan, and build.
...
- **Test DB isolation:** DB tests use an isolated test schema, never the dev/prod Prisma singleton (a suite once silently hit localhost and proved nothing). If DB tests are flag-gated (e.g. `RUN_DB_IT`), that flag must be ON in the gate that decides "done" -- a skipped test guards nothing.
...
- **Record the real (orchestrator-assigned) model on each subagent artifact** (audit/proposal/review). Don't trust a self-label or filename -- "by Claude" in a file proves nothing about what ran. No verifiable model = no proof of independent multi-model coverage.
- **Converging competing proposals = a debate, not a silent pick.** When two+ subagents return rival proposals for the same decision, run the **debate-to-consensus loop**: re-spawn the SAME model that wrote each proposal (the one place you reuse models -- each defends its own), and round by round each must defend with reasons, concede, or propose a third option. Feed each the **full running debate log every turn** (subagents are stateless -- without the accumulated log they reopen settled points and never converge); loop until consensus (or a stalemate you resolve: canonical audit wins, else stronger reasoning, else ask the user). Cap ~3-4 rounds. Log the whole back-and-forth with attribution. The converged result must reflect the consensus, not one model's unilateral pick.
